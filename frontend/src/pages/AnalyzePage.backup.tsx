...
const AnalyzePage = () => {
  ...
  const flowChartData = useMemo(() => {
    if (!result) return [];
    const map: Record<string, { timestamp: string; inflow: number; outflow: number }> = {};
    const aggregate = (timestamp: string, key: 'inflow' | 'outflow', amount: number) => {
      const day = new Date(timestamp).toLocaleDateString('es-ES');
      if (!map[day]) {
        map[day] = { timestamp: day, inflow: 0, outflow: 0 };
      }
      map[day][key] += amount;
    };
    result.flowHistory?.inflows.forEach((entry) => aggregate(entry.timestamp, 'inflow', entry.amount));
    result.flowHistory?.outflows.forEach((entry) => aggregate(entry.timestamp, 'outflow', Math.abs(entry.amount)));
    return Object.values(map).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
  }, [result]);
  ...
  return (
    <div className="dashboard-stack">
      ...
      {result && flowChartData.length > 0 && (
        <div className="panel">
          <div className="panel-header panel-header--stacked">
            <div>
              <p className="panel-label">Evolución de la cartera</p>
              <h2>Entradas vs salidas</h2>
            </div>
          </div>
          <div className="chart-wrapper">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={flowChartData} margin={{ top: 12, right: 20, left: 0, bottom: 12 }}>
                <XAxis dataKey="timestamp" />
                <YAxis />
                <Tooltip />
                <Legend />
                <Area type="monotone" dataKey="inflow" stackId="1" stroke="#22c55e" fill="#22c55e44" name="Entradas" />
                <Area type="monotone" dataKey="outflow" stackId="1" stroke="#ef4444" fill="#ef444444" name="Salidas" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {result && (
        <div className="panel">
          <div className="panel-header panel-header--stacked">
            <div>
              <p className="panel-label">Histórico de movimientos</p>
          ...
          <div className="table-wrapper">
            <table className="data-table">
              ...
              <tbody>
                {result.flowHistory.movements.map((entry) => (
                  <tr key={entry.tx_id}>
                    ...
                    <td>
                      {entry.chain || 'Binance'} / {entry.location}
                    </td>
